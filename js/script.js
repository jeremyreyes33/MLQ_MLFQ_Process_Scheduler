        ...
        for (let q = 0; q < queues.length && !executed; q++) {
            ...
            if (readyProcesses.length > 0) {
                ...
                if (selectedProcess) {
                    ...
                    const execTime = queue.algo === 'RR' ? Math.min(queue.quantum, selectedProcess.remaining) : selectedProcess.remaining;
    ...
    while (completed < totalProcesses) {
        ...
        for (let q = 0; q < queues.length && !executed; q++) {
            ...
            if (selectedProcess) {
                ...
                const execTime = algo === 'RR' ? Math.min(quantum, selectedProcess.remaining) : selectedProcess.remaining;
...
function displayResults(processes, gantt) {
    ...
    const ganttCells = gantt.map(item => {
        ...
        const textColor = item.process === 'Idle' ? '#4a5568' : 'white';
    ...
    const processRows = processes.map(p => {
        ...
        return `<tr><td style="background: ${color}; color: white; font-weight: bold;">P${p.id}</td><td>${p.arrival}</td><td>${p.burst}</td><td>${p.priority}</td><td>${p.finish}</td><td>${p.turnaround}</td><td>${p.waiting}</td></tr>`;
    }).join('');
